import React from 'react'

export default function Existcat(props) {
    console.log(props.categories)
    var colors = ['#FF6633', '#FFB399', '#FF33FF', '#FFFF99', '#00B3E6', 
        '#E6B333', '#3366E6', '#999966', '#99FF99', '#B34D4D',
        '#80B300', '#809900', '#E6B3B3', '#6680B3', '#66991A', 
        '#FF99E6', '#CCFF1A', '#FF1A66', '#E6331A', '#33FFCC',
        '#66994D', '#B366CC', '#4D8000', '#B33300', '#CC80CC', 
        '#66664D', '#991AFF', '#E666FF', '#4DB3FF', '#1AB399',
        '#E666B3', '#33991A', '#CC9999', '#B3B31A', '#00E680', 
        '#4D8066', '#809980', '#E6FF80', '#1AFF33', '#999933',
        '#FF3380', '#CCCC00', '#66E64D', '#4D80CC', '#9900B3', 
        '#E64D66', '#4DB380', '#FF4D4D', '#99E6E6', '#6666FF'];
        function Canclecat(index) {
            return () => {
                props.setCategories(props.categories.filter((cat, i) => i !== index));
            }
        }
    return (
        <>
            <h3 className='uppercase text-sm font-semibold my-3 dark:text-white'>Categories</h3>
            {props.categories.length === 0 ? <p className='text-sm font-semibold'>No categories added yet</p>:props.categories.map((cat, index) => {
                return (
                    <div key={index} className={`catslot catslot_${index} flex justify-between align-middle items-center gap-2`}>
                        <div className={`color w-5 h-4 rounded-sm`} style={{ backgroundColor: colors[index]}}></div>
                        <div className="text text-start w-full dark:text-white">{cat}</div>
                        <div className={`cancel_${index} cursor-pointer`} onClick={Canclecat(index)}>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
                                viewBox="0 0 24 24"
                                strokeWidth="1.5"
                                stroke="currentColor"
                                className="size-6"
                            >
                                <path 
                                className='dark:text-white'
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    d="M6 18 18 6M6 6l12 12"
                                />
                            </svg>
                        </div>
                    </div>
                );
            })}
        </>
    )
}